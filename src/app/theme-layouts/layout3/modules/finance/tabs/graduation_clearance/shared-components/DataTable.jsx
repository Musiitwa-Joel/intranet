...
import { GLOBAL_SEARCH_CLEARANCE_STUDENTS } from "app/theme-layouts/layout3/modules/graduation/gql/queries";
...
function DataTable() {
          ...
          return renderRow(
            ...
            <AntTypography.Link
              onClick={() => {
                ...
                dispatch(setActiveTab("student_view"));
              }}
            >
              {row.student_no}
            </AntTypography.Link>
  ...
  const onSearch = async (value) => {
    ...
    if (response.data) {
      ...
      dispatch(
        setClearanceStds(
          response?.data?.global_search_clearance_students?.results || []
        )
      );
    }
  };
